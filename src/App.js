...
function App() {
  ...
  const [inverted, setOrder] = useState(false);
  const [isEditModeActive, setEditMode] = useState(false);
  const [isModalVisible, setModalVisibility] = useState(false);

  const sortedScans = sortScansBy(sorter, scans, users, inverted);
  ...
  return (
    <div className="App">
      ...
      <Sorter sorter={sorter} setSorter={setSorter} setOrder={setOrder} inverted={inverted} />
      <ScanList
        scans={sortedScans}
        users={users}
        setUsers={setUsers}
